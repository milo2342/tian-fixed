...
A standalone GitHub-ready build of the Titan Labs Discord bot. It runs from one bundled `index.js` file and keeps persistent configuration in PostgreSQL when available, with a local JSON fallback for exported/no-Postgres mode.
...
- `vehicle-keys.json` — vehicle key catalog used by `/key-purchase`
...
`DATABASE_URL` may point to PostgreSQL for durable workspace storage. If it is not set, the exported bot uses a local JSON-backed storage fallback so commands can still operate. The original workspace contains the Drizzle schema and can push it with `pnpm --filter @workspace/db run push`; for a completely separate host, export the schema SQL from the original project before first launch.
...
Use `/key add` to add a vehicle with a price and description. The new vehicle is immediately available in `/key-purchase`. Use `/key remove` to remove a vehicle. The catalog is stored in `vehicle-keys.json`.


## Gmail payment notifications

The bot can optionally monitor a Gmail inbox for Tebex gift-card notifications and post them to the channels configured with `/key notified`. Set `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, and `GMAIL_REFRESH_TOKEN` in Railway. The watcher uses read-only Gmail access, polls every 30 seconds by default, and does not replay existing matching messages on first startup.
