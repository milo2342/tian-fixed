...
var tos = {
  data: new import_discord3.SlashCommandBuilder().setName("tos").setDescription("Show the Titan Labs vehicle key terms of service.").addSubcommand((sc) => sc.setName("show").setDescription("Post the vehicle key terms and require agreement.")).setDefaultMemberPermissions(import_discord3.PermissionFlagsBits.ManageGuild).setDMPermission(false),
  async execute(interaction) {
    if (!interaction.guild || !interaction.channel) return;
      ...
      embeds: [
        nexusEmbed({
          ...
          fields: [
            ...
            { name: "\u25C8 Tickets & Sales", value: "`/ticket-panel create|list|hub|delete` `/ticket rename` `/ticket close` `/ticket delete` `/key purchase` `/key add` `/key edit` `/key remove`" }
          ],
...
${poll2.options.map((o, i) => `**${i + 1}.** ${o} \u2014 ${tallies[i]} vote(s)`).join("\n")}`,
      ...
      case "tos_agree": {
        ...
        if (!accepted.has(interaction.user.id)) {
          ...
          setLocalGuildSettings(guildId2, { tosAcceptedUserIds: [...accepted] });
        }
        await interaction.reply({
          embeds: [nexusEmbed({ title: "Terms Accepted", description: "Thank you. Your acceptance of the Titan Labs Vehicle Key Terms has been recorded for this server.", color: THEME.success, footer: "Titan Labs • Vehicle Keys" })],
          flags: 64
        });
        return;
      }
