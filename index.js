...
// The exported bot can run without PostgreSQL. Database-backed features
// receive a clear setup error instead of crashing the process.
if (databaseConfigured) {
  getDb();
} else {
  db = new Proxy({}, { get() { throw databaseNotConfiguredError; } });
}
...
Click below to verify and unlock full server access.`,
    ...
    const message = await interaction.fetchReply();
    if (!databaseConfigured) {
      setLocalGuildSettings(interaction.guild.id, { verificationRoleId: role.id, verificationChannelId: interaction.channelId, verificationMessageId: message.id });
    } else {
...
async function handleTicketButtonLocal(interaction) {
  ...
  if (action === "ticket_open") {
    ...
    await ticketChannel.send({ content: `${interaction.user} ${pingText}`.trim(), allowedMentions: { users: [interaction.user.id], roles: panel.pingRoleIds }, embeds: [nexusEmbed({ title: panel.name, description: `${panel.welcomeMessage ? `${panel.welcomeMessage}\n${DIVIDER}\n` : ""}Thanks for opening a ticket. Support will be with you shortly.`, color: THEME.primary, footer: "Tickets & Sales" })], components: [closeRow] });
  ...
  if (action === "ticket_claim") {
    ...
    if (channel && channel.isTextBased() && "setName" in channel) {
      const slug = (interaction.member?.displayName || interaction.user.username).toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 70) || interaction.user.id;
      await channel.setName(`ticket-${slug}`).catch(() => null);
    }
...
var ticket = {
  data: new import_discord14.SlashCommandBuilder().setName("ticket").setDescription("Manage the current ticket channel.")
    .addSubcommand((sc) => sc.setName("rename").setDescription("Rename the current ticket to a custom name.").addStringOption((o) => o.setName("name").setDescription("New ticket channel name").setRequired(true)))
    .addSubcommand((sc) => sc.setName("close").setDescription("Close the current ticket and send its transcript."))
    .addSubcommand((sc) => sc.setName("delete").setDescription("Delete the current ticket channel."))
    .setDMPermission(false),
  async execute(interaction) {
    if (!interaction.guild || !interaction.channel || !interaction.channel.isTextBased()) return;
    const sub = interaction.options.getSubcommand();
    const channelId = interaction.channel.id;
    const sanitizeName = (value) => String(value).toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 90) || "ticket";
    const renameChannel = async (name) => {
      if (!("setName" in interaction.channel)) return false;
      await interaction.channel.setName(sanitizeName(name));
      return true;
    };
    if (!hasDatabase()) {
      const local = loadLocalTicketState();
      const ticketRow = local.tickets.find((t) => t.guildId === interaction.guild.id && t.channelId === channelId) ?? null;
      if (!ticketRow) {
        await replyError(interaction, "Ticket Not Found", "This channel is not registered as a ticket.");
        return;
      }
      if (sub === "rename") {
        const name = interaction.options.getString("name", true);
        await renameChannel(name);
        await replyOk(interaction, "Ticket Renamed", `Ticket renamed to **${sanitizeName(name)}**.`, void 0, "Tickets & Sales");
        return;
      }
      if (sub === "delete") {
        if (!await requireStaff(interaction)) {
          await replyError(interaction, "Access Denied", "Only staff can delete tickets.");
          return;
        }
        await interaction.reply({ embeds: [nexusEmbed({ title: "Deleting Ticket", description: "This channel will be deleted shortly.", color: THEME.danger })] });
      ...
      if (sub === "close") {
        if (ticketRow.status !== "open") {
          await replyError(interaction, "Already Closed", "This ticket is already closed.");
          return;
        }
        const isStaff = await requireStaff(interaction);
        if (interaction.user.id !== ticketRow.openerId && !isStaff) {
          await replyError(interaction, "Access Denied", "Only the ticket opener or staff can close this ticket.");
          return;
        }
        localUpdateTicket(ticketRow.id, interaction.guild.id, { status: "closed", closedAt: new Date().toISOString() });
        await interaction.channel.permissionOverwrites.edit(ticketRow.openerId, { SendMessages: false }).catch(() => null);
        const transcriptLines = [`Titan Labs ticket transcript`, `Ticket ID: ${ticketRow.id}`, `Guild: ${interaction.guild.name} (${interaction.guild.id})`, `Opened by: ${ticketRow.openerId}`, `Closed by: ${interaction.user.id}`, ""];
        const messages = await interaction.channel.messages.fetch({ limit: 100 }).catch(() => null);
        if (messages) for (const message of [...messages.values()].reverse()) transcriptLines.push(`[${message.createdAt.toISOString()}] ${message.author.tag}: ${message.content || message.embeds.map((e) => `[embed: ${e.title ?? "untitled"}]`).join(" ") || "[no text]"}`);
        const panel = localFindPanel(interaction.guild.id, ticketRow.panelId);
        const transcriptChannel = panel ? await interaction.guild.channels.fetch(panel.transcriptChannelId ?? panel.channelId).catch(() => null) : null;
        if (transcriptChannel?.isTextBased()) await transcriptChannel.send({ content: `Transcript for ticket #${ticketRow.id} closed by ${interaction.user}.`, files: [new import_discord16.AttachmentBuilder(Buffer.from(transcriptLines.join("\n"), "utf8"), { name: `ticket-${ticketRow.id}-transcript.txt` })] }).catch(() => null);
        await replyOk(interaction, "Ticket Closed", `Closed by ${interaction.user}. Staff can delete this channel when ready.`, void 0, "Tickets & Sales");
      }
      return;
    }
    const ticketRow = await db.query.discordTicketsTable.findFirst({ where: (t, { and, eq }) => and(eq(t.guildId, interaction.guild.id), eq(t.channelId, channelId)) });
    if (!ticketRow) {
      await replyError(interaction, "Ticket Not Found", "This channel is not registered as a ticket.");
      return;
    }
    ...
    if (sub === "close") {
      ...
      await db.update(discordTicketsTable).set({ status: "closed", closedAt: new Date() }).where((0, import_drizzle_orm10.eq)(discordTicketsTable.id, ticketRow.id));
      await interaction.channel.permissionOverwrites.edit(ticketRow.openerId, { SendMessages: false }).catch(() => null);
      const transcriptLines = [`Titan Labs ticket transcript`, `Ticket ID: ${ticketRow.id}`, `Guild: ${interaction.guild.name} (${interaction.guild.id})`, `Opened by: ${ticketRow.openerId}`, `Closed by: ${interaction.user.id}`, ""];
      const messages = await interaction.channel.messages.fetch({ limit: 100 }).catch(() => null);
      if (messages) for (const message of [...messages.values()].reverse()) transcriptLines.push(`[${message.createdAt.toISOString()}] ${message.author.tag}: ${message.content || message.embeds.map((e) => `[embed: ${e.title ?? "untitled"}]`).join(" ") || "[no text]"}`);
      const panel = await db.query.discordTicketPanelsTable.findFirst({ where: (0, import_drizzle_orm10.eq)(discordTicketPanelsTable.id, ticketRow.panelId) });
      const transcriptChannel = panel ? await interaction.guild.channels.fetch(panel.transcriptChannelId ?? panel.channelId).catch(() => null) : null;
      if (transcriptChannel?.isTextBased()) await transcriptChannel.send({ content: `Transcript for ticket #${ticketRow.id} closed by ${interaction.user}.`, files: [new import_discord16.AttachmentBuilder(Buffer.from(transcriptLines.join("\n"), "utf8"), { name: `ticket-${ticketRow.id}-transcript.txt` })] }).catch(() => null);
      await replyOk(interaction, "Ticket Closed", `Closed by ${interaction.user}. Staff can delete this channel when ready.`, void 0, "Tickets & Sales");
    }
...
var key = {
  data: new import_discord15.SlashCommandBuilder().setName("key").setDescription("Manage the vehicle key catalog.").addSubcommand(
    ...
    (sc) => sc.setName("edit").setDescription("Edit a vehicle key's name, price, or description.")
      .addStringOption((o) => o.setName("name").setDescription("Current vehicle name").setRequired(true).setAutocomplete(true))
...
` : ""}Thanks for opening a ticket. Support will be with you shortly.`,
      ...
      case "ticket_claim": {
        ...
        const claimChannel = interaction.channel;
        if (claimChannel && claimChannel.isTextBased() && "setName" in claimChannel) {
          const slug = (interaction.member?.displayName || interaction.user.username).toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 70) || interaction.user.id;
          await claimChannel.setName(`ticket-${slug}`).catch(() => null);
        }
        const updatedEmbed = interaction.message.embeds[0] ? import_discord16.EmbedBuilder.from(interaction.message.embeds[0]).setColor(THEME.accent).addFields({ name: "Assigned Support", value: `${interaction.user} is handling this ticket.` }) : nexusEmbed({ title: "Ticket Claimed", description: `${interaction.user} is now handling this ticket.`, color: THEME.accent });
