...
var developer = {
  ...
  async execute(interaction) {
    ...
    if (sub === "add" || sub === "remove") {
      ...
      if (!database) {
        await replyError(interaction, "Database Setup Required", "Adding or removing developers requires PostgreSQL. Set DATABASE_URL in your hosting environment and restart the bot.");
        return;
      }
      const target = interaction.options.getUser("user", true);
      if (sub === "add") {
    ...
    const developers = database ? await database.query.discordDevelopersTable.findMany() : [];
...
var keyPurchaseCommands = [keyPurchase, key];
