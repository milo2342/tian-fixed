...
async function executeTicketPanelLocal(interaction) {
  ...
  if (sub === "hub") {
    ...
    const emojis = ["❓", "🚓", "💳", "✈️", "🎫"];
    const buttons = panels.map((panel, i) => new import_discord14.ButtonBuilder().setCustomId(`ticket_open:${panel.id}`).setLabel(panel.name.slice(0, 80)).setStyle(styles[i]).setEmoji(emojis[i]));
    const message = await channel.send({ embeds: [nexusEmbed({ title, description: `${description}\n\n${DIVIDER}\nSelect a department below to open a private ticket. Please choose the closest match so your request reaches the right team.`, color: THEME.accent, fields: panels.map((panel, i) => ({ name: `${emojis[i]} ${panel.name}`, value: String(panel.embedDescription || "").slice(0, 900), inline: false })), footer: "Tickets & Sales • Titan Labs Support" })], components: [new import_discord14.ActionRowBuilder().addComponents(buttons)] });
...
var key = {
  ...
  async autocomplete(interaction) {
    if (interaction.options.getSubcommand(false) !== "purchase") {
      await interaction.respond([]);
      return;
    }
    const focused = interaction.options.getFocused().toLowerCase();
