...
var pool = null;
var databaseConfigured = Boolean(process.env.DATABASE_URL);
var databaseNotConfiguredError = Object.assign(new Error("DATABASE_URL is not configured. This feature requires PostgreSQL."), { code: "DATABASE_NOT_CONFIGURED" });

// Local JSON-backed database fallback. This lets the exported bot run without
// PostgreSQL while preserving the same small subset of Drizzle operations the
// bot uses (query, insert, update, delete, conflict handling and predicates).
var LOCAL_DB_FILE = import_path.join(process.cwd(), "local-db.json");
var localStore = null;
function loadLocalStore() {
  if (localStore) return localStore;
  try {
    if (import_fs.existsSync(LOCAL_DB_FILE)) {
      const parsed = JSON.parse(import_fs.readFileSync(LOCAL_DB_FILE, "utf8"));
      if (parsed && typeof parsed === "object") localStore = parsed;
    }
  } catch (err) {
    logger?.warn?.({ err }, "local database could not be read");
  }
  if (!localStore || typeof localStore !== "object") localStore = {};
  return localStore;
}
function saveLocalStore() {
  try {
    import_fs.writeFileSync(LOCAL_DB_FILE, JSON.stringify(localStore, null, 2));
  } catch (err) {
    logger?.warn?.({ err }, "local database could not be written");
  }
}
function tableKey(table) {
  try {
    for (const [exportName, exported] of Object.entries(schema_exports)) {
      if (exported === table && exportName.endsWith("Table")) return exportName;
    }
  } catch {}
  return table?.[Symbol.for("drizzle:Name")] || table?.[Symbol.for("drizzle:Columns")]?.__name || "table";
}
...
function fieldKey(table, column) {
  if (column && column.__localField) return column.__localField;
  for (const key of Object.keys(table || {})) {
    if (table[key] === column) return key;
  }
  if (column?.name) {
    const target = String(column.name);
    for (const key of Object.keys(table || {})) {
...
function makePredicate(table, where) {
  if (!where) return () => true;
  if (typeof where === "function") return where(table, LOCAL_OPS);
  if (typeof where === "function") return where;
  return () => true;
}
const LOCAL_OPS = {
  eq: (column, value) => row => row[fieldKeyCached(column)] === unwrap(value),
  and: (...predicates) => row => predicates.filter(Boolean).every(p => p(row)),
  or: (...predicates) => row => predicates.filter(Boolean).some(p => p(row)),
  ilike: (column, value) => {
    const needle = String(value).toLowerCase().replace(/%/g, "");
    return row => String(row[fieldKeyCached(column)] ?? "").toLowerCase().includes(needle);
  },
  inArray: (column, values) => row => Array.isArray(values) && values.map(unwrap).includes(row[fieldKeyCached(column)]),
};
var currentPredicateTable = null;
function fieldKeyCached(column) {
  return column?.__localField || column?.__localFieldKey || (column && column.__table ? fieldKey(column.__table, column) : column?.name);
}
function unwrap(value) { return value && value.__localValue !== undefined ? value.__localValue : value; }
function withLocalColumns(table) {
  const clone = table;
  for (const key of Object.keys(table || {})) {
    const col = table[key];
    if (col && typeof col === "object") {
      try { col.__localField = key; col.__table = table; } catch {}
    }
  }
  return clone;
}
function applyLocalDefaults(table, row) {
  const out = { ...row };
  const name = String(tableKey(table));
  if (out.id == null && Object.prototype.hasOwnProperty.call(table, "id")) {
    const rows = rowsFor(table); out.id = rows.reduce((m, r) => Math.max(m, Number(r.id) || 0), 0) + 1;
  }
  if (out.createdAt == null && Object.prototype.hasOwnProperty.call(table, "createdAt")) out.createdAt = new Date().toISOString();
  if (out.addedAt == null && Object.prototype.hasOwnProperty.call(table, "addedAt")) out.addedAt = new Date().toISOString();
  if (name.includes("ticket_panels") && out.ticketCounter == null) out.ticketCounter = 0;
  if (name.includes("tickets") && out.status == null) out.status = "open";
  if (name.includes("giveaways")) { if (out.entrantIds == null) out.entrantIds = []; if (out.ended == null) out.ended = false; }
  if (name.includes("polls")) { if (out.votes == null) out.votes = {}; if (out.ended == null) out.ended = false; }
  if (name.includes("suggestions")) { if (out.status == null) out.status = "pending"; if (out.upvotes == null) out.upvotes = 0; if (out.downvotes == null) out.downvotes = 0; }
  if (name.includes("reports") && out.status == null) out.status = "pending";
  if (name.includes("loa_requests") && out.status == null) out.status = "pending";
  if (name.includes("warnings") && out.acknowledged == null) out.acknowledged = false;
  if (name.includes("guild_settings") && out.logEvents == null) out.logEvents = [];
  return out;
}
function localQuery(table) {
  withLocalColumns(table);
  return {
    async findMany(options = {}) {
      const pred = makePredicate(table, options.where);
      return rowsFor(table).filter(r => pred(r)).map(r => ({ ...r }));
    },
    async findFirst(options = {}) {
      const pred = makePredicate(table, options.where);
      const row = rowsFor(table).find(r => pred(r));
      return row ? { ...row } : undefined;
    }
  };
}
function localInsert(table) {
  withLocalColumns(table);
  let values = []; let conflict = null; let conflictNothing = false;
  const builder = {
    values(input) { values = Array.isArray(input) ? input : [input]; return builder; },
    onConflictDoUpdate(cfg) { conflict = cfg; return builder; },
    onConflictDoNothing() { conflictNothing = true; return builder; },
    async returning() { return execute(); },
    then(resolve, reject) { return execute().then(resolve, reject); },
    catch(reject) { return execute().catch(reject); }
  };
  async function execute() {
    const rows = rowsFor(table); const results = [];
    for (const value of values) {
      const row = applyLocalDefaults(table, value);
      let existingIndex = -1;
      if (conflict?.target) {
        const targets = Array.isArray(conflict.target) ? conflict.target : [conflict.target];
        existingIndex = rows.findIndex(existing => targets.every(col => existing[fieldKey(table, col)] === row[fieldKey(table, col)]));
      }
      if (existingIndex >= 0) {
        if (conflict?.set) rows[existingIndex] = { ...rows[existingIndex], ...conflict.set };
        else if (!conflictNothing) rows[existingIndex] = { ...rows[existingIndex], ...row };
        results.push({ ...rows[existingIndex] });
      } else { rows.push(row); results.push({ ...row }); }
    }
    saveLocalStore(); return results;
  }
  return builder;
}
function localUpdate(table) {
  withLocalColumns(table);
  let patch = {}; let where;
  const builder = { set(input) { patch = input || {}; return builder; }, where(input) { where = input; return builder; }, async returning() { return execute(); }, then(resolve, reject) { return execute().then(resolve, reject); }, catch(reject) { return execute().catch(reject); } };
  async function execute() { const rows = rowsFor(table); const pred = makePredicate(table, where); const changed=[]; for (let i=0;i<rows.length;i++){ if(pred(rows[i])){ rows[i]={...rows[i],...patch}; changed.push({...rows[i]}); }} saveLocalStore(); return changed; }
  return builder;
}
function localDelete(table) {
  withLocalColumns(table);
  let where;
  const builder = { where(input) { where = input; return builder; }, async returning() { return execute(); }, then(resolve, reject) { return execute().then(resolve, reject); }, catch(reject) { return execute().catch(reject); } };
  async function execute() { const rows=rowsFor(table); const pred=makePredicate(table, where); const removed=rows.filter(r=>pred(r)); const kept=rows.filter(r=>!pred(r)); const key=tableKey(table); localStore[key]=kept; saveLocalStore(); return removed.map(r=>({...r})); }
  return builder;
}
function createLocalDb() {
  const query = new Proxy({}, { get(_t, prop) { const table = globalThis[prop]; return localQuery(table || schema_exports[prop]); } });
  return { query, insert: table => localInsert(table), update: table => localUpdate(table), delete: table => localDelete(table) };
}
var localDb = null;
var db = null;
...
var LOCAL_SETTINGS_FILE = import_path.join(process.cwd(), "guild-settings.json");
var localGuildSettings = null;
function loadLocalGuildSettings() {
  if (localGuildSettings) return localGuildSettings;
  try {
    if (import_fs.existsSync(LOCAL_SETTINGS_FILE)) {
      const parsed = JSON.parse(import_fs.readFileSync(LOCAL_SETTINGS_FILE, "utf8"));
      if (parsed && typeof parsed === "object") {
        localGuildSettings = parsed;
        return localGuildSettings;
      }
    }
  } catch (err) {
    logger?.warn?.({ err }, "local guild settings could not be read");
  }
  localGuildSettings = {};
  return localGuildSettings;
}
function getLocalGuildSettings(guildId) {
  return { ...(loadLocalGuildSettings()[guildId] || {}) };
}
function setLocalGuildSettings(guildId, patch) {
  const all = loadLocalGuildSettings();
  all[guildId] = { ...(all[guildId] || {}), ...patch };
  import_fs.writeFileSync(LOCAL_SETTINGS_FILE, JSON.stringify(all, null, 2));
  return all[guildId];
}

// PostgreSQL is optional in the exported build; localDb is used when it is unavailable.
...
async function getSettings(guildId2) {
  if (!databaseConfigured) return getLocalGuildSettings(guildId2);
...
var tos = {
  data: new import_discord3.SlashCommandBuilder().setName("tos").setDescription("Show and configure the Titan Labs vehicle key terms of service.").addSubcommand((sc) => sc.setName("show").setDescription("Post the vehicle key terms and require agreement.")).addSubcommand((sc) => sc.setName("role").setDescription("Set the role awarded when a member agrees to the terms.").addRoleOption((o) => o.setName("role").setDescription("Role to give members who accept the TOS").setRequired(true))).setDefaultMemberPermissions(import_discord3.PermissionFlagsBits.ManageGuild).setDMPermission(false),
  async execute(interaction) {
    if (!interaction.guild || !interaction.channel) return;
    ...
    if (sub === "role") {
      ...
      if (role.managed || role.id === interaction.guild.roles.everyone.id) {
        await replyError(interaction, "Invalid Role", "Choose a normal, unmanaged role. You cannot use @everyone or an integration-managed role.");
        return;
      }
      const botMember = interaction.guild.members.me;
      if (botMember && role.position >= botMember.roles.highest.position) {
        await replyError(interaction, "Role Too High", "Move my highest role above the TOS Accepted role first.");
        return;
      }
      setLocalGuildSettings(interaction.guild.id, { tosRoleId: role.id });
      await replyOk(interaction, "TOS Role Set", `Members who click **I Agree to the Terms** will receive ${role}.`, void 0, "Server Management");
      return;
    }
    const agreement = new import_discord3.ButtonBuilder().setCustomId("tos_agree").setLabel("I Agree to the Terms").setStyle(import_discord3.ButtonStyle.Success).setEmoji("\u2705");
    const row = new import_discord3.ActionRowBuilder().addComponents(agreement);
    const embed2 = nexusEmbed({
      title: "Titan Labs — Vehicle Key Terms of Service",
      description: `Please read the following terms carefully before purchasing or receiving a vehicle key.\n${DIVIDER}\n**By clicking \"I Agree to the Terms\", you confirm that you understand and accept these rules.**\n\nThese terms also apply automatically when you use **/key purchase** or notify a staff member that you are claiming a vehicle key.`,
      color: THEME.accent,
      fields: [
        { name: "Vehicle Key Rules", value: [
          "• If you VDM in any vehicle of mine, your keys will be taken.",
          "• All sales are final.",
          "• Running from cops in speed-boosted cars is grounds for removal.",
          "• If you break any rules, your vehicle keys may be removed.",
          "• Vehicle keys remain subject to staff and server rules at all times."
        ].join("\n") },
        { name: "Gift Card & Contact", value: "Gift card email: **tyrone.psrp@gmail.com**" },
        { name: "Agreement", value: "Click **I Agree to the Terms** below to confirm your acceptance." }
      ],
      footer: "Titan Labs • Vehicle Keys"
    });
    await interaction.reply({ embeds: [embed2], components: [row] });
  }
};
...
var serverlog = {
  ...
  async execute(interaction) {
    ...
    if (!databaseConfigured) {
      setLocalGuildSettings(interaction.guild.id, { logChannelId: channel.id, logEvents: events });
    } else {
...
// artifacts/discord-bot/src/commands/welcome.ts
var import_discord4 = require("discord.js");
var welcomeConfigFile = import_path.join(process.cwd(), "welcome-settings.json");
var welcomeConfigs = {};
try {
  if (import_fs.existsSync(welcomeConfigFile)) {
    welcomeConfigs = JSON.parse(import_fs.readFileSync(welcomeConfigFile, "utf8"));
  }
} catch (err) {
  logger.warn({ err }, "could not load welcome settings");
  welcomeConfigs = {};
}
function saveWelcomeConfigs() {
  try {
    import_fs.writeFileSync(welcomeConfigFile, JSON.stringify(welcomeConfigs, null, 2), "utf8");
  } catch (err) {
    logger.error({ err }, "could not save welcome settings");
  }
}
...
var welcome = {
  data: new import_discord4.SlashCommandBuilder()
    .setName("welcome")
    .setDescription("Configure the channel and message used to welcome new members.")
    .addChannelOption((o) => o.setName("channel").setDescription("Channel where welcome messages will be sent").addChannelTypes(import_discord4.ChannelType.GuildText).setRequired(true))
    .addStringOption((o) => o.setName("message").setDescription("Welcome message. Use {user}, {username}, {server}, or {memberCount}."))
    .setDefaultMemberPermissions(import_discord4.PermissionFlagsBits.ManageGuild)
  ...
  async execute(interaction) {
    ...
    if (message.length > 1900) {
      await replyError(interaction, "Message Too Long", "The welcome message must be 1900 characters or fewer.");
      return;
    }
    if (!channel.isTextBased() || channel.isDMBased()) {
      await replyError(interaction, "Invalid Channel", "Please select a server text channel.");
      return;
    }
    welcomeConfigs[interaction.guild.id] = { channelId: channel.id, message };
    saveWelcomeConfigs();
    await replyOk(
      interaction,
      "Welcome System Configured",
      `New members will now be welcomed in ${channel}.`,
      [{ name: "Message", value: message.slice(0, 1024) }, { name: "Placeholders", value: "{user} {username} {server} {memberCount}" }],
      "Community"
    );
  }
};
function registerWelcomeEvents(client2) {
  client2.on("guildMemberAdd", async (member) => {
    ...
    const content = renderWelcomeMessage(config.message, member);
    await channel.send({ content, allowedMentions: { users: [member.id] } }).catch((err) => logger.error({ err, guildId: member.guild.id }, "welcome message failed"));
  });
}
...
var announcementRebrand = {
  ...
  async execute(interaction) {
    ...
    if (!databaseConfigured) {
      setLocalGuildSettings(interaction.guild.id, { announcementColor: String(parsed), announcementFooter: footer ?? void 0, announcementIconUrl: iconUrl ?? void 0 });
    } else {
    ...
    await interaction.reply({
      embeds: [
        nexusEmbed({
          ...
          color: THEME.primary,
          description: `${DIVIDER}\nAll currently available commands are listed below.`,
          fields: [
            { name: "\u25C8 Moderation", value: "`/ban` `/kick` `/mute` `/unmute` `/warn` `/purge` `/ban-check` `/whois` `/global-ban` `/global-unban`" },
            { name: "\u25C8 Community", value: "`/giveaway` `/poll` `/announce` `/tos show` `/invite` `/suggestion` `/confession` `/serverlog` `/welcome`" },
            { name: "\u25C8 Server Management", value: "`/lock` `/unlock` `/slowmode` `/nick` `/embed` `/embed-edit` `/steal-emoji` `/create-roles` `/create-channels` `/add-role` `/mass-move-roles` `/move-channels` `/announcement-rebrand` `/say` `/auto-role set|disable|status` `/role-menu`" },
            { name: "\u25C8 Info & Utilities", value: "`/ping` `/server-info` `/role-info` `/avatar` `/userinfo` `/developer info|add|remove` `/features` `/dm` `/dm-list add|remove|view`" },
            { name: "\u25C8 Staff & Administration", value: "`/rules` `/report` `/loa` `/staff-duty` `/whitelist`" },
            { name: "\u25C8 Tickets & Sales", value: "`/ticket-panel create|list|hub|delete` `/ticket rename` `/ticket close` `/ticket delete` `/key purchase` `/key add` `/key edit` `/key remove` `/key notified add|remove|list|clear`" }
          ],
          footer: "Info & Utilities • Titan Labs • Updated automatically"
        })
      ]
    });
  }
...
var testCommand = {
  data: new import_discord6.SlashCommandBuilder().setName("test").setDescription("Preview the Tebex gift-card payment notification.").setDefaultMemberPermissions(import_discord6.PermissionFlagsBits.ManageGuild).setDMPermission(false),
  async execute(interaction) {
    await interaction.reply({
      embeds: [nexusEmbed({
        title: "PSRP Giftcard Received",
        description: "**TEST ONLY** — This is a preview of the automatic Gmail/Tebex payment notification.",
        color: THEME.success,
        fields: [
          { name: "Amount", value: "$30.00 USD", inline: true },
          { name: "Subject", value: "Your Paradise State Roleplay gift card", inline: false },
          { name: "From", value: "Paradise State Roleplay <noreply@tebex.io>", inline: false },
          { name: "Time", value: new Date().toLocaleString(), inline: false }
        ],
        footer: "Titan Labs • Gmail Payment Monitor"
      })]
    });
  }
};
var infoCommands = [ping, serverInfo, roleInfo, avatar, userinfo, features, testCommand];
...
var rules = {
  ...
  async execute(interaction) {
    ...
    if (text17) {
      if (!databaseConfigured) {
        setLocalGuildSettings(interaction.guild.id, { rulesText: text17 });
      } else {
...
var staffDuty = {
  ...
  async execute(interaction) {
    ...
    const existing = databaseConfigured ? await db.query.discordStaffDutyTable.findFirst({ where: (0, import_drizzle_orm3.eq)(discordStaffDutyTable.guildUserId, key) }) : getLocalGuildSettings(interaction.guild.id).staffDuty?.[interaction.user.id] ? { onDuty: getLocalGuildSettings(interaction.guild.id).staffDuty[interaction.user.id] } : null;
    const row = new import_discord6.ActionRowBuilder().addComponents(
...
var whitelist = {
  ...
  async execute(interaction) {
    ...
    if (requirementsInput || applyInfoInput) {
      ...
      if (!databaseConfigured) {
        setLocalGuildSettings(interaction.guild.id, {
...
var autoRole = {
  ...
  async execute(interaction) {
    ...
    if (sub === "set") {
      ...
      if (!databaseConfigured) setLocalGuildSettings(guildId2, { autoRoleId: role.id });
      else await db.insert(discordGuildSettingsTable).values({ guildId: guildId2, autoRoleId: role.id }).onConflictDoUpdate({ target: discordGuildSettingsTable.guildId, set: { autoRoleId: role.id } });
      await replyOk(interaction, "Auto-Role Set", `New members will automatically receive <@&${role.id}>.`, void 0, "Server Management");
      return;
    }
    if (sub === "disable") {
      if (!databaseConfigured) setLocalGuildSettings(guildId2, { autoRoleId: null });
      else await db.insert(discordGuildSettingsTable).values({ guildId: guildId2, autoRoleId: null }).onConflictDoUpdate({ target: discordGuildSettingsTable.guildId, set: { autoRoleId: null } });
    ...
    const settings = await getSettings2(guildId2);
...
var developer = {
  ...
  async execute(interaction) {
    ...
    if (sub === "add" || sub === "remove") {
      ...
      const target = interaction.options.getUser("user", true);
      if (sub === "add") {
        await database.insert(discordDevelopersTable).values({ userId: target.id, addedBy: interaction.user.id }).onConflictDoNothing();
        await replyOk(interaction, "Developer Added", `${target} has been added to the developer team.`, void 0, "Info & Utilities");
        return;
      }
      await database.delete(discordDevelopersTable).where((0, import_drizzle_orm7.eq)(discordDevelopersTable.userId, target.id));
    ...
    const developers = await database.query.discordDevelopersTable.findMany();
...
// Local ticket fallback for exports without PostgreSQL.
var LOCAL_TICKET_FILE = import_path.join(process.cwd(), "ticket-data.json");
var localTicketState = null;
function loadLocalTicketState() {
  if (localTicketState) return localTicketState;
  try {
    if (import_fs.existsSync(LOCAL_TICKET_FILE)) {
      const parsed = JSON.parse(import_fs.readFileSync(LOCAL_TICKET_FILE, "utf8"));
      if (parsed && typeof parsed === "object") {
        localTicketState = {
          nextPanelId: Number(parsed.nextPanelId) > 0 ? Number(parsed.nextPanelId) : 1,
          nextTicketId: Number(parsed.nextTicketId) > 0 ? Number(parsed.nextTicketId) : 1,
    ...
    logger?.warn?.({ err }, "local ticket state could not be read");
  }
  localTicketState = { nextPanelId: 1, nextTicketId: 1, panels: [], tickets: [] };
  return localTicketState;
}
function saveLocalTicketState() {
  import_fs.writeFileSync(LOCAL_TICKET_FILE, JSON.stringify(localTicketState ?? loadLocalTicketState(), null, 2));
}
function localPanelsForGuild(guildId) {
  return loadLocalTicketState().panels.filter((p) => p.guildId === guildId);
}
function localFindPanel(guildId, id) {
  return localPanelsForGuild(guildId).find((p) => p.id === Number(id)) ?? null;
}
function localFindTicket(guildId, id) {
  return loadLocalTicketState().tickets.find((t) => t.guildId === guildId && t.id === Number(id)) ?? null;
}
function localFindOpenTicket(guildId, panelId, openerId) {
  return loadLocalTicketState().tickets.find((t) => t.guildId === guildId && t.panelId === Number(panelId) && t.openerId === openerId && t.status === "open") ?? null;
}
function localCreatePanel(data) {
  const state = loadLocalTicketState();
  const panel = { id: state.nextPanelId++, ticketCounter: 0, messageId: null, ...data };
  state.panels.push(panel);
  saveLocalTicketState();
  return panel;
}
function localUpdatePanel(id, patch) {
  const panel = loadLocalTicketState().panels.find((p) => p.id === Number(id));
  if (!panel) return null;
  Object.assign(panel, patch);
  saveLocalTicketState();
  return panel;
}
function localDeletePanel(id, guildId) {
  const state = loadLocalTicketState();
  const index = state.panels.findIndex((p) => p.id === Number(id) && p.guildId === guildId);
  if (index < 0) return null;
  const [panel] = state.panels.splice(index, 1);
  state.tickets = state.tickets.filter((t) => t.panelId !== panel.id);
  saveLocalTicketState();
  return panel;
}
function localCreateTicket(data) {
  const state = loadLocalTicketState();
  const ticket = { id: state.nextTicketId++, claimedBy: null, status: "open", closedAt: null, ...data };
  state.tickets.push(ticket);
  saveLocalTicketState();
  return ticket;
}
function localUpdateTicket(id, guildId, patch) {
  const ticket = localFindTicket(guildId, id);
  if (!ticket) return null;
  Object.assign(ticket, patch);
  saveLocalTicketState();
  return ticket;
}
async function executeTicketPanelLocal(interaction) {
  const guild = interaction.guild;
  const sub = interaction.options.getSubcommand();
  if (sub === "list") {
    const panels = localPanelsForGuild(guild.id);
    if (!panels.length) {
    ...
    await interaction.reply({ embeds: [nexusEmbed({
      title: "Ticket Panels", color: THEME.accent, description: DIVIDER,
      fields: panels.map((p) => ({
        name: `#${p.id} — ${p.name}`,
  ...
  if (sub === "hub") {
    ...
    const title = interaction.options.getString("title") ?? "Support & Purchase Tickets";
    const description = interaction.options.getString("description") ?? "Choose the department you need below. Your ticket will be private and routed to the right support team.";
    const panelIds = [...new Set(panelIdsInput.split(",").map((v) => Number(v.trim())).filter(Number.isInteger))];
    if (!panelIds.length || panelIds.length > 5) {
      await replyError(interaction, "Invalid Panel IDs", "Enter between 1 and 5 panel IDs separated by commas, for example `1,2,3`.");
      return;
    }
    const panels = panelIds.map((id) => localFindPanel(guild.id, id)).filter(Boolean);
    ...
    const styles = [import_discord14.ButtonStyle.Success, import_discord14.ButtonStyle.Primary, import_discord14.ButtonStyle.Secondary, import_discord14.ButtonStyle.Danger, import_discord14.ButtonStyle.Primary];
    const buttons = panels.map((panel, i) => new import_discord14.ButtonBuilder().setCustomId(`ticket_open:${panel.id}`).setLabel(panel.name.slice(0, 80)).setStyle(styles[i]));
    const message = await channel.send({ embeds: [nexusEmbed({ title, description: `${description}\n\n${DIVIDER}\nSelect a department below to open a private ticket. Please choose the closest match so your request reaches the right team.`, color: THEME.accent, fields: panels.map((panel) => ({ name: panel.name, value: String(panel.embedDescription || "").slice(0, 900), inline: false })), footer: "Tickets & Sales • Titan Labs Support" })], components: [new import_discord14.ActionRowBuilder().addComponents(buttons)] });
    await replyOk(interaction, "Ticket Hub Posted", `Posted a polished ticket hub in ${channel} with ${panels.length} department button(s). Message ID: \`${message.id}\``, void 0, "Tickets & Sales");
    return;
  }
  if (sub === "delete") {
    const id = interaction.options.getInteger("id", true);
    const panel = localFindPanel(guild.id, id);
    if (!panel) {
    ...
    if (panel.messageId) {
      ...
      if (channel?.isTextBased()) await channel.messages.delete(panel.messageId).catch(() => null);
    }
    localDeletePanel(id, guild.id);
    await replyOk(interaction, "Panel Deleted", `Ticket panel **${panel.name}** has been removed.`, void 0, "Tickets & Sales");
    return;
  }
  ...
  const panel = localCreatePanel({ guildId: guild.id, channelId: channel.id, transcriptChannelId: transcriptChannel?.id ?? null, name, embedTitle: title, embedDescription: description, embedColor: colorInput ? `#${color.toString(16).padStart(6, "0")}` : "#22d3ee", buttonLabel, buttonEmoji: buttonEmoji ?? null, categoryId: category?.id ?? null, accessRoleIds, pingRoleIds, ticketNamePrefix: ticketPrefix, welcomeMessage: welcomeMessage ?? null });
  const embed = nexusEmbed({ title, description: `${description}\n\n${DIVIDER}\nUse the menu below to choose an action, or press **${buttonLabel}** to start immediately.`, color, fields: [{ name: "Private Support", value: "Only you and the configured support roles can see your ticket.", inline: true }, { name: "Transcripts", value: transcriptChannel ? `Saved in ${transcriptChannel}` : "Saved in this panel channel", inline: true }], footer: "Tickets & Sales • Interactive Support" });
  ...
  const menu = new import_discord14.StringSelectMenuBuilder().setCustomId(`ticket_menu:${panel.id}`).setPlaceholder("Choose a ticket action").addOptions({ label: "Open a Ticket", description: "Create a private support channel", value: "open", emoji: "🎫" }, { label: "How It Works", description: "See what happens after opening a ticket", value: "info", emoji: "ℹ️" });
  const message = await channel.send({ embeds: [embed], components: [new import_discord14.ActionRowBuilder().addComponents(menu), row] });
  localUpdatePanel(panel.id, { messageId: message.id });
  const warnings = [unresolvedAccess.length ? `Skipped unrecognized access roles: ${unresolvedAccess.join(", ")}` : "", unresolvedPing.length ? `Skipped unrecognized ping roles: ${unresolvedPing.join(", ")}` : "", buttonEmoji && !isSafeButtonEmoji(buttonEmoji) ? "Skipped invalid button emoji; use a real Unicode or custom Discord emoji." : ""].filter(Boolean).join("\n");
  await replyOk(interaction, "Ticket Panel Created", `Posted panel **${name}** (ID #${panel.id}) in ${channel}.${warnings ? `\n${warnings}` : ""}`, void 0, "Tickets & Sales");
}
async function handleTicketButtonLocal(interaction) {
  ...
  if (action === "ticket_open") {
    const panel = localFindPanel(interaction.guild.id, id);
    if (!panel) return void await interaction.reply({ embeds: [errorEmbedLocal("Panel Unavailable", "This ticket panel no longer exists.")], flags: 64 });
    const existing = localFindOpenTicket(interaction.guild.id, panel.id, interaction.user.id);
    if (existing) return void await interaction.reply({ embeds: [errorEmbedLocal("Ticket Already Open", `You already have an open ticket: <#${existing.channelId}>.`)], flags: 64 });
    await interaction.deferReply({ flags: 64 });
    const number = panel.ticketCounter + 1;
    localUpdatePanel(panel.id, { ticketCounter: number });
    const overwrites = [
      { id: interaction.guild.roles.everyone.id, deny: [import_discord16.PermissionFlagsBits.ViewChannel] },
      { id: interaction.user.id, type: import_discord16.OverwriteType.Member, allow: [import_discord16.PermissionFlagsBits.ViewChannel, import_discord16.PermissionFlagsBits.SendMessages, import_discord16.PermissionFlagsBits.ReadMessageHistory] },
      ...panel.accessRoleIds.map((roleId) => ({ id: roleId, type: import_discord16.OverwriteType.Role, allow: [import_discord16.PermissionFlagsBits.ViewChannel, import_discord16.PermissionFlagsBits.SendMessages, import_discord16.PermissionFlagsBits.ReadMessageHistory] }))
    ];
    const botId = interaction.client.user?.id;
    if (botId) overwrites.push({ id: botId, type: import_discord16.OverwriteType.Member, allow: [import_discord16.PermissionFlagsBits.ViewChannel, import_discord16.PermissionFlagsBits.SendMessages, import_discord16.PermissionFlagsBits.ManageChannels] });
    const ticketChannel = await interaction.guild.channels.create({ name: `${panel.ticketNamePrefix}-${number}`, type: import_discord16.ChannelType.GuildText, parent: panel.categoryId ?? void 0, permissionOverwrites: overwrites });
    const ticket = localCreateTicket({ guildId: interaction.guild.id, panelId: panel.id, channelId: ticketChannel.id, openerId: interaction.user.id });
    const closeRow = new import_discord16.ActionRowBuilder().addComponents(new import_discord16.ButtonBuilder().setCustomId(`ticket_claim:${ticket.id}`).setLabel("Claim").setStyle(import_discord16.ButtonStyle.Secondary), new import_discord16.ButtonBuilder().setCustomId(`ticket_close:${ticket.id}`).setLabel("Close Ticket").setStyle(import_discord16.ButtonStyle.Danger));
    const pingText = panel.pingRoleIds.length ? panel.pingRoleIds.map((r) => `<@&${r}>`).join(" ") : "";
    await ticketChannel.send({ content: `${interaction.user} ${pingText}`.trim(), embeds: [nexusEmbed({ title: panel.name, description: `${panel.welcomeMessage ? `${panel.welcomeMessage}\n${DIVIDER}\n` : ""}Thanks for opening a ticket. Support will be with you shortly.`, color: THEME.primary, footer: "Tickets & Sales" })], components: [closeRow] });
  ...
  if (action === "ticket_claim") {
    if (!await requireStaff(interaction)) return void await interaction.reply({ embeds: [errorEmbedLocal("Access Denied", "Only staff can claim tickets.")], flags: 64 });
    const ticket = localFindTicket(interaction.guild.id, id);
    if (!ticket || ticket.status !== "open") return void await interaction.reply({ embeds: [errorEmbedLocal("Ticket Closed", "This ticket can no longer be claimed.")], flags: 64 });
    if (ticket.claimedBy) return void await interaction.reply({ embeds: [errorEmbedLocal("Already Claimed", `This ticket is already being handled by <@${ticket.claimedBy}>.`)], flags: 64 });
    localUpdateTicket(id, interaction.guild.id, { claimedBy: interaction.user.id });
    ...
    const rows = new import_discord16.ActionRowBuilder().addComponents(new import_discord16.ButtonBuilder().setCustomId(`ticket_claim:${id}`).setLabel("Claimed").setStyle(import_discord16.ButtonStyle.Secondary).setDisabled(true), new import_discord16.ButtonBuilder().setCustomId(`ticket_close:${id}`).setLabel("Close Ticket").setStyle(import_discord16.ButtonStyle.Danger));
    await interaction.update({ embeds: [updatedEmbed], components: [rows] });
    return;
  }
  if (action === "ticket_close") {
    const ticket = localFindTicket(interaction.guild.id, id);
    if (!ticket || ticket.status !== "open") return void await interaction.reply({ embeds: [errorEmbedLocal("Already Closed", "This ticket is already closed.")], flags: 64 });
    const isStaff = await requireStaff(interaction);
    if (interaction.user.id !== ticket.openerId && !isStaff) return void await interaction.reply({ embeds: [errorEmbedLocal("Access Denied", "Only the ticket opener or staff can close this ticket.")], flags: 64 });
    localUpdateTicket(id, interaction.guild.id, { status: "closed", closedAt: new Date().toISOString() });
    const channel = interaction.channel;
    if (channel?.isTextBased() && "permissionOverwrites" in channel) await channel.permissionOverwrites.edit(ticket.openerId, { SendMessages: false }).catch(() => null);
    const transcriptLines = [`Titan Labs ticket transcript`, `Ticket ID: ${ticket.id}`, `Guild: ${interaction.guild.name} (${interaction.guild.id})`, `Opened by: ${ticket.openerId}`, `Closed by: ${interaction.user.id}`, ""];
    if (channel?.isTextBased() && "messages" in channel) {
      const messages = await channel.messages.fetch({ limit: 100 }).catch(() => null);
      if (messages) for (const message of [...messages.values()].reverse()) transcriptLines.push(`[${message.createdAt.toISOString()}] ${message.author.tag}: ${message.content || message.embeds.map((e) => `[embed: ${e.title ?? "untitled"}]`).join(" ") || "[no text]"}`);
    }
    const panel = localFindPanel(interaction.guild.id, ticket.panelId);
    ...
    if (transcriptChannel?.isTextBased()) await transcriptChannel.send({ content: `Transcript for ticket #${ticket.id} closed by ${interaction.user}.`, files: [new import_discord16.AttachmentBuilder(Buffer.from(transcriptLines.join("\n"), "utf8"), { name: `ticket-${ticket.id}-transcript.txt` })] }).catch((err) => logger.warn({ err, ticketId: ticket.id }, "ticket transcript delivery failed"));
    const deleteRow = new import_discord16.ActionRowBuilder().addComponents(new import_discord16.ButtonBuilder().setCustomId(`ticket_delete:${id}`).setLabel("Delete Ticket").setStyle(import_discord16.ButtonStyle.Danger));
    await interaction.reply({ embeds: [nexusEmbed({ title: "Ticket Closed", description: `Closed by ${interaction.user}. Staff can delete this channel when ready.`, color: THEME.warning })], components: [deleteRow] });
    return;
  }
  if (action === "ticket_delete") {
    if (!await requireStaff(interaction)) return void await interaction.reply({ embeds: [errorEmbedLocal("Access Denied", "Only staff can delete tickets.")], flags: 64 });
    ...
    const channel = interaction.channel;
    setTimeout(() => { if (channel && "delete" in channel) channel.delete().catch(() => null); }, 5e3);
  }
}
async function handleTicketMenuLocal(interaction) {
  if (!interaction.guild) return;
  const panelId = Number(interaction.customId.split(":")[1]);
  if (!Number.isInteger(panelId)) return;
  const panel = localFindPanel(interaction.guild.id, panelId);
  if (!panel) return void await interaction.reply({ embeds: [nexusEmbed({ title: "Panel Unavailable", description: "This ticket panel no longer exists.", color: THEME.danger })], flags: 64 });
  if (interaction.values[0] === "info") return void await interaction.reply({ embeds: [nexusEmbed({ title: "How Tickets Work", description: `${DIVIDER}\n1. Choose **Open a Ticket** from the menu.\n2. Confirm the request.\n3. A private channel is created for you and the support team.\n4. Staff can claim, close, and archive the ticket.\n\nPlease include all relevant details in your first message so the team can help quickly.`, color: THEME.accent, footer: "Tickets & Sales • Help" })], flags: 64 });
  const row = new import_discord17.ActionRowBuilder().addComponents(new import_discord17.ButtonBuilder().setCustomId(`ticket_open:${panel.id}`).setLabel(`Open ${panel.name} Ticket`.slice(0, 80)).setStyle(import_discord17.ButtonStyle.Primary).setEmoji("🎫"));
  await interaction.reply({ embeds: [nexusEmbed({ title: "Ready to Open a Ticket?", description: `A private **${panel.name}** ticket will be created for you and the configured support roles.`, color: THEME.success, footer: "Tickets & Sales" })], components: [row], flags: 64 });
}
...
var ticketPanelCreate = {
  ...
  async execute(interaction) {
    if (!interaction.guild) return;
    if (!hasDatabase()) return executeTicketPanelLocal(interaction);
    const sub = interaction.options.getSubcommand();
    const guild = interaction.guild;
    if (sub === "list") {
...
Tickets opened: ${p.ticketCounter}`
    ...
    if (sub === "hub") {
      ...
      const buttons = orderedPanels.map(
        (panel2, index) => new import_discord14.ButtonBuilder().setCustomId(`ticket_open:${panel2.id}`).setLabel(panel2.name.slice(0, 80)).setStyle(buttonStyles[index])
      );
...
function readKeyCatalog() {
  try {
    if (!fs_key_catalog.existsSync(KEY_CATALOG_FILE)) {
      fs_key_catalog.writeFileSync(KEY_CATALOG_FILE, JSON.stringify(DEFAULT_KEY_CATALOG, null, 2));
      return [...DEFAULT_KEY_CATALOG];
    }
    const parsed = JSON.parse(fs_key_catalog.readFileSync(KEY_CATALOG_FILE, "utf8"));
    if (!Array.isArray(parsed) || parsed.length === 0) {
      fs_key_catalog.writeFileSync(KEY_CATALOG_FILE, JSON.stringify(DEFAULT_KEY_CATALOG, null, 2));
      return [...DEFAULT_KEY_CATALOG];
    }
    return parsed.filter((item) => item && typeof item.name === "string" && Number.isFinite(Number(item.price))).map((item) => ({
      name: item.name.trim(),
      price: Number(item.price),
      description: typeof item.description === "string" ? item.description : ""
    })).filter((item) => item.name);
  } catch (err) {
    logger.warn({ err }, "vehicle key catalog could not be read; using defaults");
    return [...DEFAULT_KEY_CATALOG];
  }
}
function writeKeyCatalog(catalog) {
  fs_key_catalog.writeFileSync(KEY_CATALOG_FILE, JSON.stringify(catalog, null, 2));
}
function findCatalogKey(name) {
  const focused = String(name).trim().toLowerCase();
  return readKeyCatalog().find((item) => item.name.toLowerCase() === focused) ?? null;
}
function getKeyNotificationChannels(guildId) {
  const settings = getLocalGuildSettings(guildId);
  return Array.isArray(settings.keyNotificationChannelIds) ? settings.keyNotificationChannelIds : [];
}
function setKeyNotificationChannels(guildId, channelIds) {
  return setLocalGuildSettings(guildId, { keyNotificationChannelIds: [...new Set(channelIds)] });
}
function keyPurchaseComponents() {
  const button = new import_discord15.ButtonBuilder()
    .setCustomId("key_paid")
    .setLabel("Sent Gift Card")
    .setStyle(import_discord15.ButtonStyle.Success)
    .setEmoji("\u2705");
  return [new import_discord15.ActionRowBuilder().addComponents(button)];
}
var keyPurchase = {
  data: new import_discord15.SlashCommandBuilder().setName("key-purchase").setDescription("Select a vehicle key to purchase and get the store link.").addStringOption(
    (o) => o.setName("vehicle").setDescription("Vehicle key you want").setRequired(true).setAutocomplete(true)
  ...
  async autocomplete(interaction) {
    ...
    const catalog = readKeyCatalog();
    const matches = catalog.filter((item) => !focused || item.name.toLowerCase().includes(focused)).slice(0, 25);
    await interaction.respond(matches.map((item) => ({
      name: `${item.name} — $${item.price}`.slice(0, 100),
      value: item.name.slice(0, 100)
    })));
  },
  async execute(interaction) {
    if (!interaction.guild) return;
    const tosSettings = getLocalGuildSettings(interaction.guild.id);
    if (!(tosSettings.tosAcceptedUserIds || []).includes(interaction.user.id)) {
      await replyError(interaction, "Terms Required", "Please use `/tos show` and click **I Agree to the Terms** before purchasing or claiming a vehicle key.");
      return;
    }
    ...
    const row = findCatalogKey(selection);
    if (!row) {
      ...
      await interaction.reply({
        embeds: [
          nexusEmbed({
            ...
            description: `**${row.name}** is marked as a giveaway win — no purchase needed!\n${DIVIDER}\nA staff member will verify your win in this ticket and hand over the key free of charge.`,
            color: THEME.success,
            fields: row.description ? [{ name: "Description", value: row.description }] : [],
            footer: "Tickets & Sales"
          })
        ]
      });
    ...
    await interaction.reply({
      embeds: [
        nexusEmbed({
          title: "Key Purchase",
          description: `${DIVIDER}\nPurchase a gift card for the amount below at the store, then a staff member will deliver your key.\n\n**Once you have sent the gift card, click \"Sent Gift Card\" below. This alerts the configured sellers/ownership channels that payment has been sent for verification.**`,
          color: THEME.primary,
          fields: [
            { name: "Selected", value: row.name, inline: true },
            { name: "Price", value: `$${row.price}`, inline: true },
            ...(row.description ? [{ name: "Description", value: row.description }] : []),
            { name: "Gift Card Email", value: GIFTCARD_EMAIL, inline: true },
            { name: "Store", value: STORE_URL }
          ],
          footer: "Tickets & Sales"
        })
      ],
      components: keyPurchaseComponents()
    });
  }
};
var key = {
  data: new import_discord15.SlashCommandBuilder().setName("key").setDescription("Manage the vehicle key catalog.").addSubcommand(
    (sc) => sc.setName("add").setDescription("Add a vehicle key to the purchasable catalog.").addStringOption((o) => o.setName("name").setDescription("Vehicle name").setRequired(true)).addIntegerOption((o) => o.setName("price").setDescription("Price in dollars").setRequired(true).setMinValue(1)).addStringOption((o) => o.setName("description").setDescription("Vehicle description").setRequired(true))
  ).addSubcommand(
    (sc) => sc.setName("remove").setDescription("Remove a vehicle key from the catalog.").addStringOption((o) => o.setName("name").setDescription("Exact vehicle name to remove").setRequired(true))
  ).addSubcommand(
    (sc) => sc.setName("edit").setDescription("Edit a vehicle key's name, price, or description.")
      .addStringOption((o) => o.setName("name").setDescription("Current vehicle name").setRequired(true))
      .addStringOption((o) => o.setName("new-name").setDescription("New vehicle name"))
      .addIntegerOption((o) => o.setName("price").setDescription("New price in dollars").setMinValue(1))
      .addStringOption((o) => o.setName("description").setDescription("New vehicle description"))
  ).addSubcommand(
    (sc) => sc.setName("notified").setDescription("Configure channels that receive gift-card payment alerts.")
      .addStringOption((o) => o.setName("action").setDescription("Add, remove, list, or clear notification channels").setRequired(true).addChoices(
        { name: "Add", value: "add" },
        { name: "Remove", value: "remove" },
        { name: "List", value: "list" },
        { name: "Clear", value: "clear" }
      ))
      .addChannelOption((o) => o.setName("channel").setDescription("Channel to add/remove from gift-card alerts").addChannelTypes(import_discord15.ChannelType.GuildText)),
  ).addSubcommand(
    (sc) => sc.setName("purchase").setDescription("Select a vehicle key to purchase and get the store link.")
      .addStringOption((o) => o.setName("vehicle").setDescription("Vehicle key you want").setRequired(true).setAutocomplete(true))
      .addBooleanOption((o) => o.setName("giveaway").setDescription("Did you win this vehicle from a giveaway?").setRequired(true))
  ).setDMPermission(false),
  async autocomplete(interaction) {
    const sub = interaction.options.getSubcommand(false);
    const focusedOption = interaction.options.getFocused(true);
    if (sub !== "purchase" && sub !== "edit" && sub !== "remove") {
      await interaction.respond([]);
      return;
    }
  ...
  async execute(interaction) {
    ...
    const sub = interaction.options.getSubcommand();
    const catalog = readKeyCatalog();
    if (["add", "remove", "edit", "notified"].includes(sub)) {
      const member = await interaction.guild.members.fetch(interaction.user.id).catch(() => null);
      if (!member?.permissions.has(import_discord15.PermissionFlagsBits.ManageGuild)) {
        await replyError(interaction, "Access Denied", "You need **Manage Server** to manage the vehicle key catalog and payment alerts.");
        return;
      }
    }
    if (sub === "add") {
      ...
      const entry = { name, price, description };
      if (existingIndex >= 0) catalog[existingIndex] = entry;
      else catalog.push(entry);
      writeKeyCatalog(catalog);
      await replyOk(interaction, existingIndex >= 0 ? "Vehicle Key Updated" : "Vehicle Key Added", `**${name}** — $${price} is now available in \`/key purchase\`.${description ? `\nDescription: ${description}` : ""}`, void 0, "Tickets & Sales");
      return;
    }
    if (sub === "edit") {
      const currentName = interaction.options.getString("name", true).trim();
      const newName = interaction.options.getString("new-name")?.trim();
      const newPrice = interaction.options.getInteger("price");
      const newDescription = interaction.options.getString("description");
      const index = catalog.findIndex((item) => item.name.toLowerCase() === currentName.toLowerCase());

      if (index < 0) {
        await replyError(interaction, "Vehicle Not Found", `No vehicle named **${currentName}** exists in the catalog.`);
        return;
      }
      if (!newName && newPrice === null && newDescription === null) {
        await replyError(interaction, "Nothing To Edit", "Provide at least one of `new-name`, `price`, or `description`.");
        return;
      }

      if (newName) {
        const duplicate = catalog.some((item, i) => i !== index && item.name.toLowerCase() === newName.toLowerCase());
        if (duplicate) {
          await replyError(interaction, "Name Already Exists", `A vehicle named **${newName}** already exists.`);
          return;
        }
      }

      const updated = { ...catalog[index] };
      if (newName) updated.name = newName;
      if (newPrice !== null) updated.price = newPrice;
      if (newDescription !== null) updated.description = newDescription.trim();
      ...
      await replyOk(
        interaction,
        "Vehicle Key Updated",
        `**${updated.name}** is now configured at **$${updated.price}**.${updated.description ? `\nDescription: ${updated.description}` : ""}`,
        void 0,
        "Tickets & Sales"
      );
      return;
    ...
    if (sub === "notified") {
      ...
      const guildId2 = interaction.guild.id;
      const channels = getKeyNotificationChannels(guildId2);
      if (action === "list") {
        const resolved = channels.map((id) => `<#${id}>`).join("\n") || "No notification channels configured.";
        await replyOk(interaction, "Gift Card Alert Channels", resolved, void 0, "Tickets & Sales");
        return;
      }
      if (action === "clear") {
        setKeyNotificationChannels(guildId2, []);
        await replyOk(interaction, "Gift Card Alerts Cleared", "No channels will receive gift-card payment alerts.", void 0, "Tickets & Sales");
        return;
      }
      if (!channel || channel.type !== import_discord15.ChannelType.GuildText) {
        await replyError(interaction, "Channel Required", "Choose a text channel for gift-card payment alerts.");
        return;
      }
      if (action === "add") {
        if (!channels.includes(channel.id)) channels.push(channel.id);
        setKeyNotificationChannels(guildId2, channels);
        await replyOk(interaction, "Gift Card Alerts Enabled", `${channel} will now receive gift-card payment alerts.`, void 0, "Tickets & Sales");
        return;
      }
      if (action === "remove") {
        setKeyNotificationChannels(guildId2, channels.filter((id) => id !== channel.id));
        await replyOk(interaction, "Gift Card Alerts Updated", `${channel} will no longer receive gift-card payment alerts.`, void 0, "Tickets & Sales");
        return;
      }
      return;
    }
    ...
    const name = interaction.options.getString("name", true).trim();
    if (catalog.length === 1 && catalog[0].name.toLowerCase() === name.toLowerCase()) {
      await replyError(interaction, "Cannot Remove Last Vehicle", "Keep at least one vehicle in the catalog.");
      return;
    }
    const next = catalog.filter((item) => item.name.toLowerCase() !== name.toLowerCase());
    if (next.length === catalog.length) {
      await replyError(interaction, "Vehicle Not Found", `No vehicle named **${name}** exists in the catalog.`);
      return;
    }
    writeKeyCatalog(next);
    await replyOk(interaction, "Vehicle Key Removed", `**${name}** has been removed from \`/key purchase\`.`, void 0, "Tickets & Sales");
  }
};
var keyPurchaseCommands = [key];
...
async function handleButtonInteraction(interaction) {
  if (!hasDatabase() && interaction.customId.startsWith("ticket_")) return handleTicketButtonLocal(interaction);
  const [action, idRaw] = interaction.customId.split(":");
  const id = idRaw ? Number(idRaw) : void 0;
  try {
    switch (action) {
...
${poll2.options.map((o, i) => `**${i + 1}.** ${o} \u2014 ${tallies[i]} vote(s)`).join("\n")}`,
      ...
      case "staffduty_off": {
        ...
        const key = `${interaction.guild.id}:${interaction.user.id}`;
        if (!databaseConfigured) {
          const current = getLocalGuildSettings(interaction.guild.id);
          setLocalGuildSettings(interaction.guild.id, { staffDuty: { ...(current.staffDuty || {}), [interaction.user.id]: onDuty } });
        } else {
      ...
      case "key_paid": {
        ...
        if (!interaction.member || !("permissions" in interaction.member) || !interaction.member.permissions.has(import_discord16.PermissionFlagsBits.ManageGuild)) {
          await interaction.reply({ embeds: [errorEmbedLocal("Access Denied", "Only staff with Manage Server can confirm gift-card payments.")], flags: 64 });
          return;
        }
        const channels = getKeyNotificationChannels(interaction.guild.id);
        const embed = interaction.message.embeds?.[0];
        const fields = embed?.fields ?? [];
        const selected = fields.find((f) => f.name === "Selected")?.value ?? "Unknown vehicle";
        const price = fields.find((f) => f.name === "Price")?.value ?? "Unknown price";
        const email = fields.find((f) => f.name === "Gift Card Email")?.value ?? GIFTCARD_EMAIL;
        const destinations = [];
        for (const channelId of channels) {
          const channel = await interaction.guild.channels.fetch(channelId).catch(() => null);
          if (channel?.isTextBased()) destinations.push(channel);
        }
        if (destinations.length === 0) {
          await interaction.reply({ embeds: [errorEmbedLocal("No Alert Channels", "No gift-card notification channels are configured. Use `/key notified` first.")], flags: 64 });
          return;
        }
        const alert = nexusEmbed({
          title: "Gift Card Sent — Payment Verification Required",
          description: `${interaction.user} has confirmed that the gift card for **${selected}** has been sent. **Please verify the gift card/payment before handing over the vehicle key.**`,
          color: THEME.warning,
          fields: [
            { name: "Customer", value: `<@${interaction.user.id}>`, inline: true },
            { name: "Vehicle", value: selected, inline: true },
            { name: "Price", value: price, inline: true },
            { name: "Gift Card Email", value: email, inline: true }
          ],
          footer: "Titan Labs • Sales / Ownership Alert"
        });
        for (const channel of destinations) {
          await channel.send({ embeds: [alert], allowedMentions: { users: [interaction.user.id] } }).catch((err) => logger.warn({ err, channelId: channel.id }, "gift-card alert delivery failed"));
        }
        const disabled = new import_discord16.ActionRowBuilder().addComponents(
          new import_discord16.ButtonBuilder().setCustomId("key_paid").setLabel("Gift Card Sent • Alerted Staff").setStyle(import_discord16.ButtonStyle.Secondary).setEmoji("\u2705").setDisabled(true)
        );
      ...
      case "tos_agree": {
        if (!interaction.guild) return;
        const guildId2 = interaction.guild.id;
        const current = getLocalGuildSettings(guildId2);
        const accepted = new Set(current.tosAcceptedUserIds || []);
        const alreadyAccepted = accepted.has(interaction.user.id);
        accepted.add(interaction.user.id);
        const patch = { tosAcceptedUserIds: [...accepted] };
        const tosRoleId = current.tosRoleId;
        let roleText = "No TOS role has been configured.";
        if (tosRoleId) {
          const role = interaction.guild.roles.cache.get(tosRoleId);
          const botMember = interaction.guild.members.me;
          if (role && !role.managed && botMember && role.position < botMember.roles.highest.position) {
            await interaction.member.roles.add(role).then(() => {
              roleText = `You have also been given ${role}.`;
            }).catch((err) => {
              logger.warn({ err, guildId: guildId2, roleId: tosRoleId, userId: interaction.user.id }, "TOS role assignment failed");
              roleText = "Your acceptance was recorded, but I could not assign the configured TOS role.";
            });
          } else {
            roleText = "Your acceptance was recorded, but the configured TOS role cannot currently be assigned.";
          }
        }
        setLocalGuildSettings(guildId2, patch);
        const description = alreadyAccepted
          ? `${interaction.user} had already accepted the Titan Labs Vehicle Key Terms. ${roleText}`
          : `${interaction.user} has agreed to the Titan Labs Vehicle Key Terms. ${roleText}`;
        const channelReply = await interaction.reply({
          embeds: [nexusEmbed({ title: "Terms Accepted", description, color: THEME.success, footer: "Titan Labs • Vehicle Keys" })]
        });
        return;
      }
      case "giveaway_enter": {
...
` : ""}Thanks for opening a ticket. Support will be with you shortly.`,
    ...
    if (!interaction.replied && !interaction.deferred) {
      ...
      await interaction.reply({
        embeds: [errorEmbedLocal(databaseError ? "Database Setup Required" : "Something Went Wrong", databaseError ? "This command could not complete because its storage backend is unavailable. Try again or configure DATABASE_URL for durable PostgreSQL storage." : "Please try again.")],
        flags: 64
      }).catch(() => null);
    }
  }
...
async function handleTicketMenuSelect(interaction) {
  if (!hasDatabase()) return handleTicketMenuLocal(interaction);
  const panelId = Number(interaction.customId.split(":")[1]);
  if (!Number.isInteger(panelId)) return;
...
async function handleInteraction(interaction) {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      const databaseError = err && err.code === "DATABASE_NOT_CONFIGURED";
      const embed2 = nexusEmbed({
        title: databaseError ? "Database Setup Required" : "Command Failed",
        description: databaseError ? "This feature needs PostgreSQL. Set DATABASE_URL in your hosting environment and restart the bot." : "Something went wrong while running this command.",
        color: THEME.danger
      });
      if (interaction.deferred || interaction.replied) {
...
// Local auto-role fallback when PostgreSQL is unavailable
function registerAutoRoleEvents(client2) {
  client2.on("guildMemberAdd", async (member) => {
    if (databaseConfigured) return;
    const settings = getLocalGuildSettings(member.guild.id);
    if (!settings?.autoRoleId) return;
    const role = member.guild.roles.cache.get(settings.autoRoleId);
    const botMember = member.guild.members.me;
    if (!role || role.managed || !botMember || role.position >= botMember.roles.highest.position) return;
    await member.roles.add(role).catch((err) => logger.warn({ err, guildId: member.guild.id }, "local auto-role assignment failed"));
  });
}
...
async function getLogChannel(client2, guildId2, event) {
  const settings = databaseConfigured
    ? await db.query.discordGuildSettingsTable.findFirst({ where: (0, import_drizzle_orm13.eq)(discordGuildSettingsTable.guildId, guildId2) })
    : getLocalGuildSettings(guildId2);
  if (!settings?.logChannelId || !Array.isArray(settings.logEvents) || !settings.logEvents.includes(event)) return null;
...
// Gmail / Tebex payment notification monitor
var GMAIL_STATE_FILE = import_path.join(process.cwd(), "gmail-payment-state.json");
var gmailMonitorState = null;
function loadGmailMonitorState() {
  if (gmailMonitorState) return gmailMonitorState;
  try {
    if (import_fs.existsSync(GMAIL_STATE_FILE)) {
      const parsed = JSON.parse(import_fs.readFileSync(GMAIL_STATE_FILE, "utf8"));
      if (parsed && typeof parsed === "object") gmailMonitorState = parsed;
    }
  } catch (err) {
    logger.warn({ err }, "gmail monitor state could not be read");
  }
  if (!gmailMonitorState) gmailMonitorState = { initialized: false, processedIds: [] };
  if (!Array.isArray(gmailMonitorState.processedIds)) gmailMonitorState.processedIds = [];
  return gmailMonitorState;
}
function saveGmailMonitorState() {
  try {
    import_fs.writeFileSync(GMAIL_STATE_FILE, JSON.stringify(gmailMonitorState, null, 2));
  } catch (err) {
    logger.warn({ err }, "gmail monitor state could not be saved");
  }
}
...
function extractGmailBody(payload) {
  ...
  const walk = (part) => {
    ...
    if (part.body?.data && (mime === "text/plain" || mime === "text/html" || !part.parts)) {
      const decoded = decodeBase64Url(part.body.data);
      if (decoded) chunks.push(mime === "text/html" ? stripHtml(decoded) : decoded);
    }
...
function parsePaymentNotification(message) {
  const headers = message.payload?.headers || [];
  const subject = headerValue(headers, "Subject") || "Tebex Gift Card Notification";
  const from = headerValue(headers, "From") || "noreply@tebex.io";
  ...
  const amountMatch = combined.match(/(?:\$\s?([0-9][0-9,]*(?:\.\d{2})?)\s*(?:USD|US\$)?|([0-9][0-9,]*(?:\.\d{2})?)\s*USD)/i);
  const amount = amountMatch ? `$${(amountMatch[1] || amountMatch[2]).replace(/,/g, "")} USD` : "Not detected";
  const isTebex = /noreply@tebex\.io|tebex|gift.?card/i.test(`${from}\n${subject}\n${body}`);
  return { subject, from, receivedAt, amount, body, isTebex, messageId: message.id };
}
async function getGmailAlertChannels(client2) {
  const seen = new Set();
  const result = [];
  for (const guild of client2.guilds.cache.values()) {
    for (const id of getKeyNotificationChannels(guild.id)) {
      if (seen.has(id)) continue;
      seen.add(id);
      const channel = await guild.channels.fetch(id).catch(() => null);
      if (channel?.isTextBased() && !channel.isDMBased()) result.push(channel);
    }
  }
  return result;
}
function buildGmailPaymentEmbed(payment) {
  return nexusEmbed({
    title: "PSRP Giftcard Received",
    description: "A new Tebex gift-card notification was received in the monitored Gmail inbox.",
    color: THEME.success,
    fields: [
      { name: "Amount", value: payment.amount, inline: true },
      { name: "Subject", value: payment.subject.slice(0, 1024) },
      { name: "From", value: payment.from.slice(0, 1024) },
      { name: "Time", value: payment.receivedAt && !Number.isNaN(payment.receivedAt.getTime()) ? payment.receivedAt.toLocaleString() : "Unknown", inline: true }
    ],
    footer: "Titan Labs • Gmail Payment Monitor"
  });
}
async function pollGmailPayments(client2) {
  const address = process.env.GMAIL_ADDRESS;
  const clientId2 = process.env.GMAIL_CLIENT_ID;
  const clientSecret2 = process.env.GMAIL_CLIENT_SECRET;
  const refreshToken = process.env.GMAIL_REFRESH_TOKEN;
  if (!address || !clientId2 || !clientSecret2 || !refreshToken) return;
  try {
    const { google: googleClient } = import_googleapis;
    const oauth2 = new googleClient.auth.OAuth2(clientId2, clientSecret2);
    oauth2.setCredentials({ refresh_token: refreshToken });
    const gmail = googleClient.gmail({ version: "v1", auth: oauth2 });
    const query = process.env.GMAIL_QUERY || "from:noreply@tebex.io newer_than:7d";
    const list = await gmail.users.messages.list({ userId: "me", q: query, maxResults: 25 });
    const ids = (list.data.messages || []).map((m) => m.id).filter(Boolean);
    const state = loadGmailMonitorState();
    if (!state.initialized && process.env.GMAIL_PROCESS_EXISTING !== "true") {
      state.processedIds = [...new Set([...state.processedIds, ...ids])].slice(-500);
      state.initialized = true;
      saveGmailMonitorState();
      logger.info({ count: ids.length }, "Gmail payment monitor initialized without replaying existing messages");
      return;
    }
    ...
    for (const id of ids.reverse()) {
      ...
      const payment = parsePaymentNotification(message.data);
      processed.add(id);
      if (!payment.isTebex) continue;
      const channels = await getGmailAlertChannels(client2);
      if (channels.length === 0) {
        logger.info({ id }, "Tebex gift-card email received but no /key notified channels are configured");
        continue;
      }
      const embed = buildGmailPaymentEmbed(payment);
      for (const channel of channels) {
        await channel.send({ embeds: [embed] }).catch((err) => logger.warn({ err, channelId: channel.id }, "Gmail payment notification delivery failed"));
      }
      logger.info({ id, amount: payment.amount, subject: payment.subject }, "Tebex gift-card email forwarded to Discord");
    }
    state.processedIds = [...processed].slice(-500);
    state.initialized = true;
    saveGmailMonitorState();
  } catch (err) {
    logger.warn({ err }, "Gmail payment monitor poll failed");
  }
}
function startGmailPaymentMonitor(client2) {
  if (!process.env.GMAIL_CLIENT_ID || !process.env.GMAIL_CLIENT_SECRET || !process.env.GMAIL_REFRESH_TOKEN) {
    logger.info("Gmail payment monitor disabled: set GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET, and GMAIL_REFRESH_TOKEN to enable it");
    return;
  }
  const interval = Math.max(10000, Number(process.env.GMAIL_POLL_MS) || 30000);
  void pollGmailPayments(client2);
  setInterval(() => void pollGmailPayments(client2), interval);
  logger.info({ intervalMs: interval, address: process.env.GMAIL_ADDRESS || "configured account" }, "Gmail payment monitor started");
}
...
async function deploySlashCommands() {
  ...
  const commandMap2 = new Map();
  for (const command of allCommands) {
    const json = command.data.toJSON();
    commandMap2.set(json.name, json);
  }
  const body = [...commandMap2.values()];
  if (guildId) {
    // Remove any stale global commands from older deployments so they do not
    // appear alongside the current guild commands as duplicates.
    ...
    logger.info({ count: body.length, guildId }, "Registered guild slash commands (instant) and cleared stale global commands");
  } else {
    await rest.put(import_discord19.Routes.applicationCommands(clientId), { body });
    logger.info({ count: body.length }, "Registered global slash commands (can take up to 1 hour to propagate)");
  }
}
async function restoreLocalGiveaways(client2) {
  if (databaseConfigured) return;
  try {
    const rows = await db.query.discordGiveawaysTable.findMany({ where: (row) => !row.ended });
    for (const row of rows) {
      const remaining = new Date(row.endsAt).getTime() - Date.now();
      if (remaining <= 0) await endGiveaway(client2, row.id);
      else setTimeout(() => void endGiveaway(client2, row.id), remaining);
    }
  } catch (err) {
    logger.warn({ err }, "local giveaway recovery failed");
  }
}
...
client.once("ready", async (readyClient) => {
  ...
  await restoreLocalGiveaways(readyClient);
  startGmailPaymentMonitor(readyClient);
  try {
